...
struct Linear_charge: View {
    ...
    var body: some View {
        ...
        .padding(.top, 20)
        Spacer()
        .navigationTitle(StringManager.shared.get("unit_linear_charge"))
        .padding()
        .onAppear {
    ...
    func convertLinearChargeDensity(value: Double, fromUnit: String, toUnit: String) -> Double? {
    ...
    func updateOutputValue(inputDouble: Double) {
        if let result = convertLinearChargeDensity(value: inputDouble, fromUnit: selectedFromUnit ?? "", toUnit: selectedToUnit ?? "") {
