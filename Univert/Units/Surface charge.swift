...
struct Surface_charge: View {
    ...
    var body: some View {
        ...
        .padding(.top, 20)
        Spacer()
        .navigationTitle(StringManager.shared.get("unit_surface_charge"))
        .padding()
        .onAppear {
    ...
    func convertSurfaceChargeDensity(value: Double, fromUnit: String, toUnit: String) -> Double? {
    ...
    func updateOutputValue(inputDouble: Double) {
        if let result = convertSurfaceChargeDensity(value: inputDouble, fromUnit: selectedFromUnit ?? "", toUnit: selectedToUnit ?? "") {
